...
M.does_table_contain_element = function(table, element)
...
M.get_nullls_sources = function(buf_ft)
  ...
  for _, source in pairs(sources) do
    ...
    for ft, active in pairs(filetypes) do
      if ft == buf_ft and active == true then
        if not M.does_table_contain_element(sources_name, source.name) then
...
M.get_all_attached_clients = function(buf_ft)
  ...
  for _, client in ipairs(clients) do
    ...
    if filetypes and vim.fn.index(filetypes, buf_ft) ~= -1 then
      if client.name == "null-ls" then
        local nls_sources = M.get_nullls_sources(buf_ft)
...
M.build_message = function(clients)
...
M.list_attached_lsp_clients = function()
  ...
  local clients = M.get_all_attached_clients(buf_ft)

  return M.build_message(clients)
end
